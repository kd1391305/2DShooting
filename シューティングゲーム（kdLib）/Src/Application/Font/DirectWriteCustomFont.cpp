...
HRESULT DirectWriteCustomFont::Init(IDXGISwapChain* swapChain)
{
	...
	result = pD2DFactory->CreateDxgiSurfaceRenderTarget(pBackBuffer.Get(), &props, &pRenderTarget);

	//エラーメッセージの取得
	_com_error err(result);
	std::wcout << L"HRESULT: 0x" << std::hex << result << L" - " << err.ErrorMessage() << std::endl;
...
// options：テキストの整形
//====================================
HRESULT DirectWriteCustomFont::DrawString(std::string str, Math::Vector2 pos, D2D1_DRAW_TEXT_OPTIONS options, bool shadow)
{
	HRESULT result = S_OK;
