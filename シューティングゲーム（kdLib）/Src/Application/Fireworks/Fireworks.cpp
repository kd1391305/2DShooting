...
void Fireworks1::Draw(float backScale)
{
	...
	for (int i = 0; i < 10; i++)
	{
		if (!m_bExploded)
		{
			scale = Math::Matrix::CreateScale(m_beforeScale * backScale - 0.01f * i);
		}
		else
		{
			scale = Math::Matrix::CreateScale(m_afterScale * backScale - 0.01f * i);
		}
	...
	if (m_lightTimer > 0)
	{
		float radius = m_texRadius * m_afterScale * backScale * randRange(0.7f, 1.2f);
		Math::Color color = m_color * 2;
		color.A(m_lightTimer * 4);
...
//
//		//寿命
//		p.m_life = randRange(1.5,2.3);		//0.45秒～0.6秒
//	}
//
//	//棒形（火）をランダムに発射
...
//
//		//寿命
//		p.m_life = randRange(1.5,2.3);		//0.5秒～0.8秒
//	}
//}
...
void Fireworks3::Draw(float backScale)
{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		SHADER.m_spriteShader.SetMatrix(scale * rotation * trans);
		for (int i = 0; i < 2; i++)SHADER.m_spriteShader.DrawTex_Src(m_tex);
	}

	//一瞬だけ光る処理
	if (m_lightTimer > 0)
	{
		float radius = m_texRadius * m_afterScale * backScale * randRange(0.7, 1.0f);
		Math::Color color = m_color * 2;
		color.A(m_lightTimer * 4);
		Light::Instance().Draw(m_pos * backScale, Math::Vector2{ radius,radius }, color);
	}
...
void Fireworks4::Draw(float backScale)
{
	...
	//花弁の描画
	{
		KdTexture* tex = TextureCache::Instance().Get("Texture/Petal.png").get();
		...
		for (auto& p : m_petalList)
		{
			...
			for (int i = 0; i < 2; i++)SHADER.m_spriteShader.DrawTex_Src(tex, p.m_color);
		}
	}
	//描画先をBuckBufferに戻す
	D3D.SetBackBuffer();
...
//===================================================
//初期化
//void Fireworks6::Init()
//{
//	m_type = Type::Trail_Buffer;
//
//	m_texRadius = 200;
//
//	//花火の火の部分を作成
//	while (m_circleList.size() < 150)
//	{
//		m_circleList.emplace_back();
//	}
//
...
//
////更新
//void Fireworks6::Update(float deltaTime)
//{
//	//弾けているときは動かない
//	if (!m_bExploded)
//	{
...
//		if (m_bExploded)
//		{
//			m_circleList[i].m_move *= 1.0f - (0.92 * deltaTime);
//
//			if(rand()%4 ==0)m_circleList[i].m_move.x *= 1.06f;
//
//			//重力
//			m_circleList[i].m_move.y -= m_particleGravity * deltaTime;
//		}
...
//
////描画
//void Fireworks6::Draw(float backScale)
//{
//	m_tex = std::make_shared<KdTexture>();
//	m_tex->CreateRenderTarget(m_texRadius * 2, m_texRadius * 2);
//	m_tex->ClearRenderTarget(Math::Color{ 0,0,0,0 });
...
//	if (m_circleActiveNum > 0)
//	{
//		m_trailBuffer.push_back(TrailBuffer(m_tex, m_pos, m_afterScale, m_color));
//	}
//	while (m_trailBuffer.size() > 4)
//	{
//		m_trailBuffer.pop_front();
//	}
//
//	Math::Matrix scaleMat, transMat;
//	Math::Color color = m_color;
//	for (int i = 0; i < m_trailBuffer.size(); i++)
//	{
//		color.A(1 - 0.04 * i);
//		scaleMat = Math::Matrix::CreateScale(m_afterScale * backScale * (1.00 - 0.008 * i));
//		transMat = Math::Matrix::CreateTranslation(m_trailBuffer[i].pos.x * backScale, (m_trailBuffer[i].pos.y + i) * backScale, 0);
//		SHADER.m_spriteShader.SetMatrix(scaleMat * transMat);
//		for (int j = 0; j < 2;j++)SHADER.m_spriteShader.DrawTex_Src(m_trailBuffer[i].tex, color);
//	}
//
//	//一瞬だけ光る処理
//	if (m_lightTimer > 0)
...
//
////花火を打ち上げる
//void Fireworks6::Shoot(Math::Vector2& startPos, Math::Vector2& startMove, float beforeScale, float afterScale, Math::Color& color)
//{
//	m_pos = startPos;
//	m_move = startMove;
//	m_beforeScale = beforeScale;
...
//		//座標を中心から少しずらす
//		p.m_pos = {
//			texPos.x + randRange(-5.f,5.f),
//			texPos.y + randRange(-5.f,5.f)
//		};
//
//		p.Update(0,m_baseScale);
//
...
//		vec.x = cosf(radian);
//		vec.y = sinf(radian);
//		p.m_move = vec * randRange(0,m_explodeSpeedMax/1.2f);
//
//		//拡縮を少し変更
//		p.m_scale = 1.0 + randRange(-0.3f, 0.3f);
//
...
//		float g = m_color.G() + randRange(-0.1f, 0.1f);
//		float b = m_color.B() + randRange(-0.1f, 0.1f);
//		float a = m_color.A() + randRange(0.f, 0.5f);
//		p.m_color = { r,g,b,a };
//
//		p.m_life = randRange(1.5, 2.3);
//	}
//
//}